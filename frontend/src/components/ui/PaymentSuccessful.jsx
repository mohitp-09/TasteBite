...
export default function PaymentSuccessful() {
  ...
  const orderData = params.get("order_data")
    ? JSON.parse(decodeURIComponent(params.get("order_data")))
    : [];
      ...
      const verifyPayment = async () => {
        try {
          ...
          const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_URL}/order/success?session_id=${sessionId}&email=${email}`
          );          
  ...
  const orderDate = orderData[0]?.date
    ? new Date(orderData[0].date)
    : new Date();
